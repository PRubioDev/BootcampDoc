...
Establecer autenticación por contraseña en root
```shell
sudo mysql -p
```
...
GRANT ALL PRIVILEGES ON *.* TO 'UserX'@'localhost' WITH GRANT OPTION;
```
...
1. Te vas a la tienda de Software de Ubuntu.
2. Buscas mysql workbench, e instalas.
3. Una vez instalado, te vas a Permissions.
4. Le das que a todo, te pedirá la contraseña.
5. Profit.



### Base de datos con la que hemos hecho pruebas
...
sudo bash install -r us-west-2 -k AKAKAKAKAKAKAK -s KAKAKAKAKAKAKA
```